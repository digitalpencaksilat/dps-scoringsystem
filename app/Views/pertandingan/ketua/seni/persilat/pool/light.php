...
.kps-header-title { font-family: 'Oswald', sans-serif; font-size: clamp(0.85rem, 2vw, 1rem); font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; text-align: center; line-height: 1.2; color: #212529; }
...
.kps-peserta-card { background: #fff; border: 1px solid #dee2e6; border-top: 3px solid #c5a017; border-radius: 10px; padding: clamp(0.6rem, 1.5vw, 0.85rem) 1rem; margin-bottom: 0.75rem; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
...
<div id="kp-seni-app">
	<div class="kps-header">
		<div class="kps-header-icon"><i class="fas fa-hand-sparkles"></i></div>
		<div><div class="kps-header-title"><?= $penampilan_seni_berlangsung->nama_seni ?? 'Seni' ?></div><div class="kps-header-sub text-center"><?= $penampilan_seni_berlangsung->nama_kategori_usia ?? '' ?> <?= ($penampilan_seni_berlangsung->jenis_kelamin ?? '') === 'Putra' ? 'Putra' : 'Putri' ?> (Pool)</div></div>
	</div>
	...
	<div class="tab-content kps-tab-content">
		...
		<div class="tab-pane" id="summary" role="tabpanel">
			...
			<div class="kps-table-wrap"><table class="kps-summary-table table-sm" id="tabelSummaryPenampilan"><thead><tr><th>#</th><th style="text-align:left;">Nama</th><th>Med Keb</th><th>Med</th><th>Pen</th><th>Final</th><th>Time</th><th>Std D</th><th>Disq</th></tr></thead><tbody><?php foreach ($semua_penampilan_seni as $idx => $pnSeni): ?><tr class="penampilan_seni_<?= $pnSeni->id_penampilan_seni ?>"><td><?= $idx+1 ?></td><td class="nama-cell"><?= str_replace('<br>',' ',$pnSeni->anggota_kelompok_peserta_seni ?? '-') ?><span class="kontingen-cell"><?= $pnSeni->nama_kontingen ?? '' ?></span></td><td class="kebenaran_median_<?= $pnSeni->id_penampilan_seni ?>"></td><td class="median_<?= $pnSeni->id_penampilan_seni ?>"></td><td class="hukuman_<?= $pnSeni->id_penampilan_seni ?>"></td><td class="nilai_akhir_<?= $pnSeni->id_penampilan_seni ?>"><?= number_format($pnSeni->nilai_akhir ?? 0, 3) ?></td><td class="waktu_<?= $pnSeni->id_penampilan_seni ?>"><?= date("i:s", $pnSeni->waktu_tampil ?? 0) ?></td><td class="standar_deviasi_<?= $pnSeni->id_penampilan_seni ?>"></td><td class="keterangan_<?= $pnSeni->id_penampilan_seni ?>"><?= ($pnSeni->diskualifikasi ?? 0) == 1 ? '<span class="kps-disq">Disq</span>' : '' ?></td></tr><?php endforeach ?></tbody></table></div>
